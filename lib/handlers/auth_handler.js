      ...
      if(!client){
        ...
        User.findByUsername(username, user => { 
          if(!user){
            ...
            getPrivateKey().then(secret => {  // Get user Roles            
               User.getUserRoles(user.userId, role=>{
                // Get user Access Menu..
                User.getAccessMenu(user.userId,(accessMenu)=>{

                  let token = tokenRepo.getToken('usr'+user.userId, secret, {user:user,roles:role,accessMenu:accessMenu});
