        ...
        User.findByUsername(username, user => { 
          if(!user){
            ...
            getPrivateKey().then(secret => {  // Get user Roles            
               User.getUserRoles(user.id, role=>{
                // Get user Access Menu..
                User.getAccessMenu(user.id,(accessMenu)=>{
                  let token = tokenRepo.getToken('usr'+user.id, secret, {user:user,roles:role,accessMenu:accessMenu});
