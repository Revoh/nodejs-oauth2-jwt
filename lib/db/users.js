...
class User {
  constructor(id, username,password,email,firstName,middleName,lastName,title,status,mobileNumber,department,belongToDfsp){
    this.id = id;
    ...
    this.belongToDfsp = belongToDfsp;
  }
...
async function findByUsername(username, cb){
  ...
  let user =  new User(userData.userId, userData.userName,userData.password, userData.email,userData.firstName,
    userData.middleName,userData.lastName,userData.title,userData.status,userData.mobileNumber,userData.department,userData.belongToDfsp);
    cb(user);
}
